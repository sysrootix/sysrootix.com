...
- Main work: retail chains end to end — storefront, mobile app, backend, CRM, POS/1C, servers. Plus own products.
...
| Medusa | client, store chain: loyalty, in-store booking, Mini App, admin, app, landings; RootDesk (shared inbox) lives in this contour | Node, React, Postgres, Swift/Expo | https://webapp.mda-platform.top |
...
Deliberately left off the site for now: cheapai.lol (API-key shop, clashes with the portfolio image), «Полчища» (no public URL yet), Finansy, Exchanger, LoalityPlus, drafts and forks. Ask before adding them.
...
- RootDesk: separate card or stays a line inside Medusa?
- «Хабаровск» only, or «Хабаровск и другие города» for the retail work?
- Public URL for «Полчища», if one appears.
